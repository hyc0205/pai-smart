...
public class EmbeddingClient {
    ...
    private String callApiOnce(List<String> batch) {
        ...
        requestBody.put("input", batch);  // 直接传入字符串数组
        requestBody.put("encoding_format", "float");
        requestBody.put("dimensions", dimension);  // 注意是 dimensions 不是 dimension

        try {
            logger.debug("发送请求到embedding API，模型: {}, 批次大小: {}, 维度: {}", modelId, batch.size(), dimension);
            ...
            logger.error("API调用失败，状态码: {}, 响应内容: {}", e.getStatusCode(), e.getResponseBodyAsString());
            throw e;
        }
    }
